...
- Use `display: flex; flex-direction: column;` on body to prevent margin collapse from breaking overflow validation. **The `flex-direction: column` is critical**  without it, multiple direct children of body (e.g., header + main content) will be placed side by side instead of stacked vertically
