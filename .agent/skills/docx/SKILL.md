---
name: docx
description: "Word document creation, editing, analysis, and manipulation. Use when: creating new Word documents (.docx), editing or reviewing existing documents with tracked changes, extracting text from documents, adding comments, creating professional structured documents, or any .docx file task."
---
...
This skill handles all tasks involving Microsoft Word (.docx) files: creating new documents, editing existing ones with tracked changes, extracting content, and adding comments or annotations.

## Design Requirements

When creating new documents:
- Use 1.3x line spacing (`250 twips`) for readability
- Center charts horizontally
- Deliver professional-quality documents with proper covers, TOC where appropriate, and consistent styling
...
for para in doc.paragraphs:
    if "keyword" in para.text:  # Find paragraphs containing specific text
        doc.add_comment(
            runs=[para.runs[0]],  # Specify the text to comment on
            text="Review needed",
            author="Your Name"  # Set comment author
        )
