...
export async function run(rawInput: unknown): Promise<{
  ...
  try { parsed = parseInput(rawInput); }
    ...
    return {
      success: false, format: "markdown", content: `❌ 输入解析失败：${err.message}`,
  ...
  if (!stocks.length) return {
    success: false, format: outputFormat, content: "❌ 未提供股票代码",
  ...
  try {
    ...
    return {
      success: false, format: outputFormat, content: `❌ 数据抓取失败：${err.message}`,
  ...
  try {
    ...
    return {
      success: false, format: outputFormat, content: `❌ 报告生成失败：${err.message}`,
      summary: { total: stocks.length, buy: 0, watch: 0, sell: 0, errors: stocks.length }, error: err.message
    };
  }
